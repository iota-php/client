...
if(isAjax())
{
    ...
    $transactions = \Techworker\IOTA\Util\TrytesUtil::arrayToTrytes(array_map('trim', array_filter(explode("\n", $_POST['transactions']))), \Techworker\IOTA\Type\Transaction::class);
        ...
        $result = $iota->getRemoteApi()->broadcastTransactions(
            $node, $transactions
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p>Broadcasts a list of transactions to all neighbors. The transactions for this
call are provided by the `attachToTangle` endpoint.</p>
        <p><pre>public function broadcastTransactions(
    Techworker\IOTA\Node $node,
    array $transactions
) : \Techworker\IOTA\RemoteApi\Commands\BroadcastTransactions\Response</pre></p>
    </div>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                        node: $("#node").val(),                                transactions: $("#transactions").val()                        };
