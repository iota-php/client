...
if(isAjax())
{
    ...
    $transactionHashes = \Techworker\IOTA\Util\TrytesUtil::arrayToTrytes(array_map('trim', array_filter(explode("\n", $_POST['transactionHashes']))), \Techworker\IOTA\Type\TransactionHash::class);
        ...
        $result = $iota->getRemoteApi()->getTrytes(
            $node, $transactionHashes
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p>Returns the raw transaction data (trytes) of a specific transaction. These
trytes can then be easily converted into the actual transaction object. See
utility functions for more details.</p>
        <p><pre>public function getTrytes(
    Techworker\IOTA\Node $node,
    array $transactionHashes
) : \Techworker\IOTA\RemoteApi\Commands\GetTrytes\Response</pre></p>
    </div>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                        node: $("#node").val(),                                transactionHashes: $("#transactionHashes").val()                        };
