...
if(isAjax())
{
    ...
    $neighborUris = array_map('trim', array_filter(explode("\n", $_POST['neighborUris'])));
    
        $result = $iota->getRemoteApi()->addNeighbors(
            $node, $neighborUris
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p>Adds one or more neighbors to the given node. This is only temporary for the
...
Most nodes obviously deny access to that endpoint.</p>
        <p><pre>public function addNeighbors(
    Techworker\IOTA\Node $node,
    array $neighborUris
) : \Techworker\IOTA\RemoteApi\Commands\AddNeighbors\Response</pre></p>
    </div>
    <div class="form-group">
        ...
        <div class="form-group">
        <label for="neighborUris">neighborUris</label>
        <textarea class="form-control" id="neighborUris" name="neighborUris" rows="3"></textarea>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                        node: $("#node").val(),                                neighborUris: $("#neighborUris").val()                        };
