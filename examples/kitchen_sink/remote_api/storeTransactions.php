...
if(isAjax())
{
    ...
    $transactions = \Techworker\IOTA\Util\TrytesUtil::arrayToTrytes(array_map('trim', array_filter(explode("\n", $_POST['transactions']))), \Techworker\IOTA\Type\Transaction::class);
        ...
        $result = $iota->getRemoteApi()->storeTransactions(
            $node, $transactions
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p>Store transactions into the local storage. The trytes to be used for this
call are returned by attachToTangle.</p>
        <p><pre>public function storeTransactions(
    Techworker\IOTA\Node $node,
    array $transactions
) : \Techworker\IOTA\RemoteApi\Commands\StoreTransactions\Response</pre></p>
    </div>
