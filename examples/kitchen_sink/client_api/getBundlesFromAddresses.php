...
if(isAjax())
{
    try {
        ...
        $inclusionStates = isset($_POST['inclusionStates']);

        $result = $iota->getClientApi()->getBundlesFromAddresses(
            $node, $addresses, $inclusionStates
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p></p>
        <p><pre>public function getBundlesFromAddresses(
    Techworker\IOTA\Node $node,
    array $addresses,
    bool $inclusionStates = 
) : \Techworker\IOTA\ClientApi\Actions\GetBundlesFromAddresses\Result</pre></p>
    </div>
    <div class="form-group">
        ...
        <label class="form-check-label">
            <input type="checkbox" id="inclusionStates" name="inclusionStates" value="1" class="form-check-input" checked="checked">
            inclusionStates
        </label>
    </div>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                                            node: $("#node").val(),                                                                addresses: $("#addresses").val(),                                                    };
                                                        if($("#inclusionStates").is(':checked')) {
            data.inclusionStates = true;
        }
