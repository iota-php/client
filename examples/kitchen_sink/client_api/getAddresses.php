...
if(isAjax())
{
    ...
    $security = \Techworker\IOTA\Type\SecurityLevel::fromValue($_POST['security']);
        ...
        $result = $iota->getClientApi()->getAddresses(
            $node, $seed, $startIndex, $amount, $addChecksum, $security
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p></p>
        <p><pre>public function getAddresses(
...
) : \Techworker\IOTA\ClientApi\Actions\GetAddresses\Result</pre></p>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control" name="amount" id="amount" value="1">
    </div>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                        node: $("#node").val(),                                seed: $("#seed").val(),                                startIndex: $("#startIndex").val(),                                amount: $("#amount").val(),                                                security: $("#security").val()                        };
