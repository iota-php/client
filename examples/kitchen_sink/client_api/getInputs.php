...
if(isAjax())
{
    ...
    $threshold = new \Techworker\IOTA\Type\Iota($_POST['threshold']);
    $security = \Techworker\IOTA\Type\SecurityLevel::fromValue($_POST['security']);

        $result = $iota->getClientApi()->getInputs(
            $node, $seed, $startIndex, $endIndex, $threshold, $security
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p></p>
        <p><pre>public function getInputs(
    Techworker\IOTA\Node $node,
    Techworker\IOTA\Type\Seed $seed,
    int $startIndex = 0,
    int $endIndex = -1,
    Techworker\IOTA\Type\Iota $threshold = ,
    Techworker\IOTA\Type\SecurityLevel $security = 
) : \Techworker\IOTA\ClientApi\Actions\GetInputs\Result</pre></p>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control" name="endIndex" id="endIndex" value="-1">
    </div>
    <div class="form-group">
        <label for="threshold">threshold</label>
        <input type="number" class="form-control" name="threshold" id="threshold" value="null">
    </div>
    <div class="form-group">
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                        node: $("#node").val(),                                seed: $("#seed").val(),                                startIndex: $("#startIndex").val(),                                endIndex: $("#endIndex").val(),                                threshold: $("#threshold").val(),                                security: $("#security").val()                        };
