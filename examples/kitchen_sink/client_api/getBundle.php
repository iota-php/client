...
if (isAjax()) {
    try {
        ...
        if ('' !== $_POST['transactionHash']) {
            $transactionHash = new \Techworker\IOTA\Type\TransactionHash($_POST['transactionHash']);
        } else {
            $transactionHash = null;
        }

        $result = $iota->getClientApi()->getBundle(
            $node,
            $transactionHash
        );
...
<main role="main" class="container">
    ...
    <div style="border: 1px solid #f0f0f0; padding: 10px; margin-bottom: 20px;">
        <p></p>
        <p><pre>public function getBundle(
    Techworker\IOTA\Node $node,
    Techworker\IOTA\Type\TransactionHash $transactionHash
) : \Techworker\IOTA\ClientApi\Actions\GetBundle\Result</pre></p>
    </div>
    ...
    <div class="form-group">
        <label for="transactionHash">transactionHash</label>
        <input type="text" class="form-control" id="transactionHash" name="transactionHash" aria-describedby="transactionHash" placeholder="" value="">
    </div>
...
<script>
    $('#submit').on('click', function(e) {
        $(".spinner").show();
        var data = {
                                            node: $("#node").val(),                                                                transactionHash: $("#transactionHash").val()                                    };
